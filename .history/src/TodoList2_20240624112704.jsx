...
const TodoList2 = () => {
  ...
  const [todos, settodo] = useState(() => {
    // Use useState to create a state variable 'todos' and a function 'setTodos' to update it.
    // The initial state is determined by the function passed to useState.
    const storage_todo = localStorage.getItem("todos");
    // Retrieve the 'todos' item from local storage.
    // This will return a string if the item exists, or null if it doesn't.
    return storage_todo ? JSON.parse(storage_todo) : [];
    // If 'storedTodos' is not null, parse the JSON string to get the array of todos.
    // If 'storedTodos' is null, return an empty array as the initial state.
  });
  ...
  useEffect(() => {
    localStorage.setItem("todos", JSON.stringify(todos));// It stores the value under the key "todos".
  }, [todos]);
  ...
  return (
    <div className="container mt-5">
      ...
      <div className="input-group">
        ...
        <button className="btn btn-secondary" onClick={reset_todo}> Reset</button>
      </div>
