...
const TodoList2 = () => {
  ...
  const updated_todo = (index) => {
    setEditing(index);//updates the Editing variable to store updated task iindex
  };
  const save_todo = () => {
    setEditing(null);//updates the Editing variable be empty so that it can store the index of the element which will be edited next.
  };
  const edit_change = (e, index) => {
    const new_todo = todos.slice();
    new_todo[index] = e.target.value;
    settodo(new_todo);
  };
